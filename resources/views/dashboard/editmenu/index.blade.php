...
<div class="container-fluid">
  <div class="fade-in">
    <div class="row">
      <div class="col-sm-12">
        <div class="card">
          <div class="card-header"><h4>Menú</h4></div>
            <div class="card-body">

                <div class="row mb-3 ml-3">
                    <a class="btn btn-lg btn-primary" href="{{ route('menu.create') }}">Agregar nuevo elemento al menú</a>
                </div>
                ...
                <div class="row mb-3">
                    <div class="col-sm-4">
                        <form action="{{ route('menu.index') }}" methos="GET">
                            ...
                            <br>
                            <button type="submit" class="btn btn-primary">Cambiar menú</button>
                        </form>
                    </div>
                </div>
<?php
    ...
    function renderDropdownForMenuEdit($data, $role){
        if(array_key_exists('slug', $data) && $data['slug'] === 'dropdown'){
            ...
            echo '</td>';
            echo '<td>';
            echo '<a class="btn btn-primary" href="' . route('menu.show', ['id' => $data['id']]) . '">Ver</a>';
            echo '</td>';
            echo '<td>';
            echo '<a class="btn btn-warning" href="' . route('menu.edit', ['id' => $data['id']]) . '">Editar</a>';
            echo '</td>';
            echo '<td>';
            echo '<a class="btn btn-danger" href="' . route('menu.delete', ['id' => $data['id']]) . '">Eliminar</a>';
            echo '</td>';
            echo '</tr>';
            ...
            for($i = 0; $i < count($data); $i++){
                if( $data[$i]['slug'] === 'link' ){
                    ...
                    echo '</td>';
                    echo '<td>';
                    echo '<a class="btn btn-primary" href="' . route('menu.show', ['id' => $data[$i]['id']]) . '">Ver</a>';
                    echo '</td>';
                    echo '<td>';
                    echo '<a class="btn btn-warning" href="' . route('menu.edit', ['id' => $data[$i]['id']]) . '">Editar</a>';
                    echo '</td>';
                    echo '<td>';
                    echo '<a class="btn btn-danger" href="' . route('menu.delete', ['id' => $data[$i]['id']]) . '">Eliminar</a>';
                    echo '</td>';
                    echo '</tr>';
                ...
                @foreach($menuToEdit as $menuel)
                    @if($menuel['slug'] === 'link')
                        <tr>
                            ...
                            <td>
                                <a class="btn btn-primary" href="{{ route('menu.show', ['id' => $menuel['id']]) }}">Ver</a>
                            </td>
                            <td>
                                <a class="btn btn-warning" href="{{ route('menu.edit', ['id' => $menuel['id']]) }}">Editar</a>
                            </td>
                            <td>
                                <a class="btn btn-danger" href="{{ route('menu.delete', ['id' => $menuel['id']]) }}">Eliminar</a>
                            </td>
                        </tr>
