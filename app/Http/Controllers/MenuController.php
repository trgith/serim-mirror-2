...
class MenuController extends Controller
{
    ...
    public function store(Request $request){
        ...
        $menulist->save();
        $request->session()->flash('message', 'Menú creado correctamente');
    ...
    public function update(Request $request){
        ...
        $menulist->name = $request->input('name');
        $menulist->save();
        $request->session()->flash('message', 'Menú actualizado correctamente');
    ...
    */
    ...
    public function delete(Request $request){
        ...
        if(!empty($menus)){
            $request->session()->flash('message', "No se puede eliminar este menú por que tiene elementos asociados ha el");
            $request->session()->flash('back', 'menu.menu.index');
            return view('dashboard.shared.universal-info');
        }else{
            Menulist::where('id', '=', $request->input('id'))->delete();
            $request->session()->flash('message', 'Menú eliminado correctamente');
            $request->session()->flash('back', 'menu.menu.index');
            return view('dashboard.shared.universal-info');
        }
    }
